...
module Inner = struct
  ...
  let fun_of_bvars (l:T.t list) (t:T.t) : T.t =
    assert (List.for_all T.is_bvar l);
    let n = List.length l in
    let m =
      l
      |> List.mapi (fun i t -> t, T.bvar ~ty:(T.ty_exn t) (n-i-1))
      |> T.Map.of_list
    in
    T.replace_m t m
  ...
  let delay ~bvars subst t1 sc1 t2 sc2 =
    if T.equal t1 t2 && sc1=sc2 then subst (* trivial *)
      ...
      let u1 = T.fun_l (env_l_dense bvars.B_vars.left) t1 in
      let u2 = T.fun_l (env_l_dense bvars.B_vars.right) t2 in
  ...
  *)
  ...
  and unif_term ~op ~root ~bvars subst t1 sc1 t2 sc2 : unif_subst =
    ...
    begin match view1, view2 with
      ...
      | T.App ({T.term=T.Const id1; _}, l1),
        ...
        ) else if op=O_unify && not root && has_non_unifiable_ty t1 then (
          ...
          | O_match_protect (P_scope sc2') | O_variant (P_scope sc2') ->
            ...
            if sc1=sc2' then (
              ...
              (* NOTE: we collapse both scopes together. The idea now
                 ...
                 variables of [t2] *)
              assert (T.Seq.vars t2 |> Sequence.for_all (fun v -> not (HVar.is_fresh v)));
              unif_ho ~op ~root ~bvars subst t1 t2 ~scope:sc2
            )
  ...
  and proj_fun ~bvars subst (t,sc_t) : unif_subst =
    ...
    begin match T.view f with
      ...
      | T.Var v ->
        if List.for_all T.is_bvar l then (
